...
import type { CanvasTapeIconStyle, CanvasTapePattern, CanvasTapeStyle } from "@/domain/model"
...
function patternForIcon(entry: TapeIconDefinition, color: string, style: CanvasTapeIconStyle): CanvasTapePattern {
  return { kind: "icon", id: entry.id, color, style }
}
...
function patternsMatch(left: CanvasTapePattern | undefined, right: CanvasTapePattern): boolean {
  ...
  if (left.kind === "icon" && right.kind === "icon") return left.id === right.id && left.color.toLowerCase() === right.color.toLowerCase() && (left.style ?? "outline") === (right.style ?? "outline")
  return false
}

function renderIconNode(node: TapeIconDefinition["nodes"][number], key: string, style: CanvasTapeIconStyle): React.ReactNode {
  const canFill = node.type !== "line" && node.type !== "polyline"
  const fill = style === "filled" && canFill ? "currentColor" : node.fill
  const strokeWidth = style === "filled" ? 1.35 : node.strokeWidth
  if (node.type === "path") return <path key={key} d={node.d} fill={fill} stroke={node.stroke} strokeWidth={strokeWidth} strokeLinecap={node.strokeLinecap} strokeLinejoin={node.strokeLinejoin} fillRule={node.fillRule} clipRule={node.clipRule} opacity={node.opacity} />
  if (node.type === "circle") return <circle key={key} cx={node.cx} cy={node.cy} r={node.r} fill={fill} stroke={node.stroke} strokeWidth={strokeWidth} strokeLinecap={node.strokeLinecap} strokeLinejoin={node.strokeLinejoin} opacity={node.opacity} />
  if (node.type === "line") return <line key={key} x1={node.x1} y1={node.y1} x2={node.x2} y2={node.y2} fill={fill} stroke={node.stroke} strokeWidth={strokeWidth} strokeLinecap={node.strokeLinecap} strokeLinejoin={node.strokeLinejoin} opacity={node.opacity} />
  if (node.type === "polyline") return <polyline key={key} points={node.points} fill={fill} stroke={node.stroke} strokeWidth={strokeWidth} strokeLinecap={node.strokeLinecap} strokeLinejoin={node.strokeLinejoin} fillRule={node.fillRule} clipRule={node.clipRule} opacity={node.opacity} />
  if (node.type === "polygon") return <polygon key={key} points={node.points} fill={fill} stroke={node.stroke} strokeWidth={strokeWidth} strokeLinecap={node.strokeLinecap} strokeLinejoin={node.strokeLinejoin} fillRule={node.fillRule} clipRule={node.clipRule} opacity={node.opacity} />
  return <rect key={key} x={node.x} y={node.y} width={node.width} height={node.height} rx={node.rx} ry={node.ry} fill={fill} stroke={node.stroke} strokeWidth={strokeWidth} strokeLinecap={node.strokeLinecap} strokeLinejoin={node.strokeLinejoin} opacity={node.opacity} />
}

function IconGlyph({ definition, color = "currentColor", style = "outline" }: { definition: TapeIconDefinition; color?: string; style?: CanvasTapeIconStyle }) {
  return <svg className="tape-pattern-icon" data-style={style} viewBox="0 0 24 24" aria-hidden="true" style={{ color }} focusable="false">{definition.nodes.map((node, index) => renderIconNode(node, `${definition.id}-${index}`, style))}</svg>
}
...
function PatternGlyph({ pattern, size = "normal" }: { pattern: CanvasTapePattern; size?: "normal" | "small" }) {
  ...
  return definition ? <IconGlyph definition={definition} color={pattern.color} style={pattern.style ?? "outline"} /> : null
}
...
function PickerContent({
  ...
  setIconStyle(style: CanvasTapeIconStyle): void
  ...
  const filteredEntries = React.useMemo(() => {
    if (activeTab === "emoji") {
      return TAPE_EMOJI_ENTRIES.filter((entry) => category === "all" || entry.category === category)
    }
    return TAPE_ICON_ENTRIES.filter((entry) => category === "all" || entry.category === category)
  }, [activeTab, category])
  ...
  const chooseIcon = (pattern: CanvasTapePattern, index: number): void => {
    ...
    setDraft((current) => ({ color: current.color, pattern: { ...pattern, style: pattern.style ?? "outline" } }))
    setActiveTab("icon")
    setSelectedTab("icon")
    setIconColor(pattern.color)
    setIconStyle(pattern.style ?? "outline")
    setFocusedIndex(index)
  }
  ...
  return (
    <div className="tape-pattern-picker" data-mode={mode}>
      ...
      {activeTab === "icon" ? <div className="tape-pattern-icon-controls">
        <div className="tape-pattern-icon-styles" role="group" aria-label="图标样式">
          {(["outline", "filled"] as const).map((style) => <button key={style} type="button" className={iconStyle === style ? "is-selected" : ""} aria-pressed={iconStyle === style} onClick={() => { setIconStyle(style); setDraft((current) => current.pattern?.kind === "icon" ? { ...current, pattern: { ...current.pattern, style } } : current) }}>{style === "outline" ? "线形" : "面形"}</button>)}
        </div>
        <div className="tape-pattern-icon-colors" aria-label="Icon colors">
      ...
      <div className="tape-pattern-catalog">
        ...
        {filteredEntries.length ? <div className="tape-pattern-grid" role="listbox" aria-label={activeTab === "emoji" ? "Emoji 图案" : "Icon 图案"}>
          {filteredEntries.map((entry, index) => {
            const pattern = activeTab === "emoji" ? patternForEmoji(entry as TapeEmojiEntry) : patternForIcon(entry as TapeIconDefinition, iconColor, iconStyle)
...
export function TapePatternPicker({ open, onOpenChange, mode, initialStyle, accountId, trigger, onConfirm }: TapePatternPickerProps) {
  ...
  const [iconStyle, setIconStyle] = React.useState<CanvasTapeIconStyle>(initial.pattern?.kind === "icon" ? initial.pattern.style ?? "outline" : "outline")
  ...
  const prepareOpen = (): void => {
    ...
    setIconStyle(next.pattern?.kind === "icon" ? next.pattern.style ?? "outline" : "outline")
  ...
  const content = <PickerContent mode={mode} draft={draft} activeTab={activeTab} selectedTab={selectedTab} category={category} recent={recent} iconColor={iconColor} iconStyle={iconStyle} setDraft={setDraft} setActiveTab={setActiveTab} setSelectedTab={setSelectedTab} setCategory={setCategory} setIconColor={setIconColor} setIconStyle={setIconStyle} markIconColorSelected={markIconColorSelected} onBackgroundChange={onBackgroundChange} confirm={confirm} onCancel={() => onOpenChange(false)} />

  if (compact) {
    return <Sheet open={open} onOpenChange={handleOpenChange}>
      <SheetPrimitive.Trigger asChild>{trigger}</SheetPrimitive.Trigger>
      <SheetContent side="bottom" showCloseButton className="tape-pattern-sheet">
        <SheetHeader><SheetTitle>定制胶带</SheetTitle><SheetDescription className="sr-only">选择胶带底色和重复图案</SheetDescription></SheetHeader>
        {content}
      </SheetContent>
    </Sheet>
  }
  ...
  return <Popover open={open} onOpenChange={handleOpenChange}>
    ...
    <PopoverContent align="center" sideOffset={10} className="tape-pattern-popover" aria-label="定制胶带">
      {content}
    </PopoverContent>
  </Popover>
}
